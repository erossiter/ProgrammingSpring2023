...
df <- data.frame("link" = vector, # put your vector from Question 1 here!
                         "office_hours" = NA)

for(i in 1:nrow(df)){
  url <- paste0(ps, df$link[i])
  html <- read_html(url)
  p_tags <- html %>%
    html_nodes('div.faculty-details') %>%
    html_nodes('p') %>%
    html_text2()
  office_hours <- which(grepl("Office Hours", p_tags))
  if(length(idx) == 0){
    next
  }else{
    df$office_hours[i] <- p_tags[idx]
  }
}
View(faculty_df)
```
