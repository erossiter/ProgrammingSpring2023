---
title: "Max Hammond, Problem Set 5"
...
knitr::opts_chunk$set(echo = TRUE,"/Users/maxhammond/Documents/Notre Dame/R/Programming/Rlab/PSO5")
knitr::opts_knit$set("/Users/maxhammond/Documents/Notre Dame/R/Programming/Rlab/PS05")
```
...
setwd('/Users/maxhammond/Documents/Notre Dame/R/Programming/Rlab')
getwd()
list.files()
```
...
``` {r, warning=FALSE}
setwd("/Users/maxhammond/Documents/Notre Dame/R/Programming/Rlab/PS01")
getwd()
list.files()
```
...
for(i in dirs){
  if (grepl(pattern = "Day", i)){
    folder_path <- file.path(i)
    cat("Folder name: ",i, "\n")
    for (j in dir(folder_path)) {
    cat("\t", j, "\n")
    }
  }
}
...
scrape_state <- function(state) {
  if (!(state %in% state.abb)){
    ...
    end_url <- "/county/S.json"
    link <- paste0(url <- paste0(base_url, state, end_url))
  flat <- jsonlite::fromJSON(link, flatten = T)
  county_df <- as.data.frame(flat$counties)
  unnest_can <- unnest(county_df, race.candidates)
}
...
for(s in state.abb){
  try(
    full_data <- rbind.data.frame(full_data,scrape_state(s)))
}
